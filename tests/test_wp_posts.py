...
class TestWordPressPosts:
    ...
    """)
    def test_create_post_and_verify_record_in_db(self, db_connection, basic_auth_wp, delete_post_fixture):
        post_data = Payloads.generate_post()
        post_id = self.api_posts.create_post(post_data, basic_auth_wp)["id"]
        post_by_id = self.db_post.select_post_by_id(db_connection, post_id)
        with allure.step("Проверить, что созданный post отображается в списке всех posts БД"):
            assert post_id in self.db_post.select_all_ids_posts(db_connection), \
        ...
        delete_post_fixture(post_id, basic_auth_wp)
    ...
    """)
    def test_create_and_del_random_count_post(self, db_connection, basic_auth_wp, delete_remaining_posts):
        db_posts_before = self.db_post.select_all_ids_posts(db_connection)
        new_posts_ids = self.api_posts.create_random_posts(basic_auth_wp)
        db_posts_after = self.db_post.select_all_ids_posts(db_connection)
        ...
        db_posts_after_del = self.db_post.select_all_ids_posts(db_connection)
        ...
        delete_remaining_posts(new_posts_ids, del_ids, basic_auth_wp)
    ...
    """)
    def test_update_post_and_verify_record_in_db(self, db_connection, basic_auth_wp, delete_post_fixture):
        post_data = Payloads.generate_post()
        post_id = self.api_posts.create_post(post_data, basic_auth_wp)["id"]
        post_db_before = self.db_post.select_post_by_id(db_connection, post_id)
        post_data_update = Payloads.generate_post()
        self.api_posts.update_post(post_data_update, basic_auth_wp, post_id)
        post_db_after = self.db_post.select_post_by_id(db_connection, post_id)
    ...
    """)
    def test_delete_post_and_verify_record_in_db(self, db_connection, basic_auth_wp, delete_remaining_posts):
        ...
        db_all_posts = self.db_post.select_all_ids_posts(db_connection)
        ...
        delete_remaining_posts(posts_ids, [random_post_id], basic_auth_wp)
    ...
    соответствующими данным введенным при создании post.
    """)
    def test_create_post_in_db_and_verify_post_data(self, db_connection, basic_auth_wp, delete_post_by_id):
        post_data = Payloads.create_post_db()
        new_post_id = self.db_post.create_post(db_connection, post_data)
        ...
        delete_post_by_id(db_connection, new_post_id)
    ...
    соответствующими данным введенным при обновлении post.
    """)
    def test_update_post_and_verify_post_data(self, db_connection, basic_auth_wp):
        post_data = Payloads.create_post_db()
        post_id = self.db_post.create_post(db_connection, post_data)
        ...
        self.db_post.update_post_by_id(db_connection, query_update_str, post_id)
    ...
    @allure.description("""
        ...
        """)
    def test_create_and_del_random_count_post_in_db(self, db_connection, basic_auth_wp, delete_remaining_posts_db):
        posts_before = self.api_posts.get_all_posts()
        ids_posts_before = ResponseConverter.extract_all_ids(posts_before)
        count_new_posts = self.db_post.create_random_posts(db_connection)
        ...
        count_del_ids = self.db_post.delete_random_posts(db_connection, new_posts_ids)
        ...
        delete_remaining_posts_db(db_connection, new_posts_ids, ids_posts_after_del)
